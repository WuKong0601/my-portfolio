  ...
  return (
    <div id='home' className='w-full h-screen flex flex-col justify-center items-center isolate relative z-10'>
      ...
      xl:mb-0 md:mb-20 ms-0 relative'>
        <h1 className='flex flex-col xl:space-y-8 md:space-y-4 space-y-2 xl:text-6xl 
        ms:text-4xl text-3xl md:font-normal font-bolder mt-8 md:mt-0'>
          <span className='flex mx-auto md:mx-0'>
            {letters.map((letter, index) => (
              <span 
                key={index} 
                className='inline-block md:w-38 w-32 xl:-mr-20 -mr-24 relative' 
                onMouseEnter={() => setHoveredLetter(index)} onMouseLeave={() => setHoveredLetter(null)}
              >
                {letter.char}
                <img
                  src={letter.img}
          ...
          <span className='xl:text-6xl md:text-4xl text-2xl tracking-wider xl:py-4 py-2 overflow-hidden text-center '>
            I'm {' '}
            <span className={`inline-block xl:w-[380px] md:w-[240px] w-[160] lg:ml-6 ml-2 font-extrabold 
        ...
        <button className='xl:w-[400px] md:w-[300px] w-[270px] bg-gray-900 dark:bg-gray-200 md:py-1 py-0 
        md:px-4 px-2 xl:text-2xl md:text-xl text-base text-white dark:text-gray-900 tracking-widest 
        rounded-r-4xl flex justify-between items-center md:mr-auto md:mx-0 mx-auto transition-colors duration-500' 
        onClick={() => setIsTextVisible(!isTextVisible)}
        onMouseEnter={() => setRoadImageOpacity(0.8)} onMouseLeave={() => setRoadImageOpacity(0.5)}>

          
          {isTextVisible ? 'Hide my story': 'Read my story'} 
          <i className={`bx ${isTextVisible ? 'bx-book-alt' : 'bx-book-open'}`}></i>
        </button>
        ...
        <img src="images/road.png" alt="Road Image" className='w-full mx-auto' style={{ opacity: roadImageOpacity }} />
        <span className='xl:text-xs md:text-[10px] text-[8px] font-bold tracking-wide 
        absolute -top-5 xl:right-22 lg:right-26 md:right-16 right-10 
        rotate-[3.5deg] animate-bounce'>
          Looking for new challenges
        </span>
        <div className={`xl:h-[150px] h-[100px] px-3 xl:text-lg md:text-base text-xs 
        font-light text-gray-900 dark:text-gray-200 text-justify tracking-wide overflow-y-auto 
        transform origin-top custom-scrollbar ${isTextVisible ? 'scale-y-100' : 'scale-y-0'}
        transition-transform duration-300`}>
          <p className='xl:py-3 py-1 px-1 [&::first-letter]:text-[30px] 
          [&::first-letter] :ml-5 [&::first-letter]:text-red-500 dark:[&::first-letter]:text-yellow-500'>{aboutText}</p>
        </div>
        </div>
      </div>
